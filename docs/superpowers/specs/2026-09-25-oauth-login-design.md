...
| Связывание | Ключ — `(provider, subject)`; при первом входе — к существующему пользователю с тем же подтверждённым e-mail | Аккаунты с паролем на tolerance.cc переживут переход: владелец входит через GitHub или Google с той же почтой |
...
2. Нет — нужен `EmailVerified`, иначе `email_unverified`. Пользователь с этим
   e-mail есть — привязать личность, аудит `user.identity_linked`. Нет — создать
...
- Интеграционные (`dbtest`): `SignIn` — новый пользователь, повторный вход,
  привязка по e-mail к пользователю из прошлой схемы, отказ неподтверждённой
