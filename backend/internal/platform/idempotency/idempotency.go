...
// after that exact crash would run the mutation again. Closing that window
// completely would mean every module's service accepting an
// externally-opened transaction instead of managing its own, which slice 1
// does not do (see docs/plans/slice-1-contract-and-access.md, "Явно
// отложено"). Two genuinely concurrent requests with the same key before
// either has recorded a result will likewise both run the mutation; the
// ON CONFLICT DO NOTHING below only prevents a corrupted second row, not a
// second side effect. Neither gap is exercised by slice 1's own commands
// (sequential retries of campaign/mission/budget commands), which is what
// the plan's acceptance test covers.
//
...
// different route with the same header value never collide.
func Command(pool *db.Pool, endpoint string, fn Mutation) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if len(key) < 8 || len(key) > 128 {
			httpx.WriteError(w, r, httpx.New(http.StatusUnprocessableEntity, "invalid_key", "Idempotency-Key должен содержать 8–128 символов."))
			return
		}
...
func lookup(ctx context.Context, pool *db.Pool, actor identity.Actor, endpoint, key, digest string) (status int, body []byte, found bool, err error) {
	txErr := pool.Tx(ctx, actor.OrganizationID, actor.UserID, func(ctx context.Context, tx pgx.Tx) error {
		...
		scanErr := tx.QueryRow(ctx, `
			...
			actor.OrganizationID, actor.UserID, endpoint, key).Scan(&storedDigest, &status, &body)
		...
		if storedDigest != digest {
			return httpx.New(http.StatusConflict, "idempotency_conflict", "Этот ключ уже использован для другого тела запроса.")
		}
		found = true
		return nil
	})
...
func save(ctx context.Context, pool *db.Pool, actor identity.Actor, endpoint, key, digest string, status int, body []byte) error {
	return pool.Tx(ctx, actor.OrganizationID, actor.UserID, func(ctx context.Context, tx pgx.Tx) error {
		_, err := tx.Exec(ctx, `
			INSERT INTO idempotency_records (organization_id, actor_id, endpoint, key, payload_digest, status, body, expires_at)
			VALUES ($1, $2, $3, $4, $5, $6, $7, now() + interval '90 days')
			ON CONFLICT (organization_id, actor_id, endpoint, key) DO NOTHING`,
			actor.OrganizationID, actor.UserID, endpoint, key, digest, status, body)
		return err
	})
}
