...
func Record(ctx context.Context, tx pgx.Tx, e Event) error {
	...
	_, err = tx.Exec(ctx, `
		INSERT INTO audit_events
			(id, actor_id, actor_kind, action, aggregate_kind, aggregate_id,
			 before_version, after_version, reason, payload, request_id)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NULLIF($9, ''), $10, NULLIF($11, ''))`,
		idgen.New("audit"), e.ActorID, kind, e.Action, e.AggregateKind, e.AggregateID,
