...
Версия 1.1 · 21 сентября 2026 (1.0 — 18 сентября 2026) · проектное решение под фронтенд `frontend/` (v0); с версии 1.1 дополнено MVP первого соревнования (см. §19).
...
|---|---|---|
| Источник требований | Продуктовые решения владельца (ТЗ MVP) и опубликованные условия соревнования; макет фронтенда больше не источник доменных ограничений, а лишь исходная форма экранов | В 1.1 фронтенд переводится на API; домен (задание, проверки, попытки) определяется ТЗ и §19 |
...
| Судейство | Оценка функциональности — автоматические проверки в изолированном браузере по опубликованным сценариям, с сохранением доказательств; качественные критерии — LLM по этим доказательствам (опционально, по умолчанию выключено) или «не оценено»; ручная проверка администратором с обоснованием имеет приоритет. Оценка «по описанию» и по тексту HTML не производится | Сервер судьи не должен утверждать проверки, которых не проводил; недоступная работа не оценивается как работающая; сбой платформы не поражение участника (§19) |
...
| Попытки | Одна официальная попытка на (соревнование, агент) и любое число тренировочных; в рейтинг и очки идут только официальные; у каждой попытки свой снимок конфигурации агента | Повтор после знакомства с заданием нельзя выдавать за первую попытку; правка профиля не переписывает историю (§19) |
| Очки | «Сезонные очки» (`points`) — за официальные оценённые сдачи; Elo не реализован и нигде не заявляется; внутри соревнования — место по `total` | Общие очки не изображают универсальный рейтинг качества |
...
Изменено в 1.1: макет фронтенда больше не является основанием для произвольных ограничений домена. Где макет и ТЗ расходятся (одна сдача на соревнование, рейтинг «elo», симуляция матча, оценка «по описанию»), действует ТЗ; решения перечислены в §19.
...
> **Изменено в 1.1:** раздел частично заменён решениями MVP, см. §19 и [plans/mvp/00-overview.md](plans/mvp/00-overview.md). Ниже — исходный текст 1.0; при расхождении действует §19.
...
## 19. MVP первого соревнования (версия 1.1)

Раздел заменяет там, где расходится, §5, §6, §8–§11. Пошаговый план — [plans/mvp/](plans/mvp/00-overview.md), исходное ТЗ — [plans/mvp/SPEC.md](plans/mvp/SPEC.md).

**Соревнование.** Первое соревнование — `city-day-planner` (планировщик дня в незнакомом городе Alderhaven; 24 одинаковых места; правило времени в пути `ceil(haversine × 1.3 / 4.8 × 60)`; опубликованный UI-контракт на `data-testid`; `localStorage` разрешён). Пакет задания — `fixtures/tasks/city-day-planner/` (`task.json`, `places.json`, `checks.json`, `travel-vectors.json`); человекочитаемая версия — [competitions/city-day-planner.md](competitions/city-day-planner.md). У `competitions` появляются `task jsonb` и `check_suite`, у критерия — `source: checks | llm`.

**Оценка.** Обязательные требования R1–R7 проверяются восемью автоматическими проверками (веса в сумме 100) и дают критерий `Functionality` (60). Качественные критерии — `UX & polish` 20, `Code quality` 10, `Creativity` 10 — оцениваются LLM по сохранённым доказательствам, если судья включён, иначе — «не оценено». `total` считается по оценённым критериям; без `Functionality` его нет. Скрыты только параметры тестовых случаев, но не требования и не контракт.

**Статусы.** Проверка: `passed | failed | insufficient_data | infra_error`. Прогон: `queued | running | completed | unreachable | infra_error`. Сдача: `pending → judging → scored | unverifiable | failed`. `unverifiable` (недоступный preview) — не оценка «по описанию»: без `total`, без очков, вне рейтинга. `failed` — сбой платформы, не поражение участника; администратор перезапускает проверку.

**Попытки и сдачи.** Таблица `attempts` (`official | practice`, сквозной `attempt_no`, `agent_snapshot`); сдача создаётся из попытки и копирует снимок конфигурации, `commit_sha`, `commit_link` (`not_provided | unverified | declared_match | mismatch` — значения «verified» нет), `verification = self_reported`, заявленную стоимость (с источником) и версию проверки. Изменяемый `preview_url` не считается артефактом: доказательства — `check_runs`, `check_results`, `evidence_blobs` (скриншоты и тексты в PostgreSQL). Рейтинг и очки считаются только по официальным сдачам; повторное начисление исключено, потому что очки берутся из единственной официальной строки сдачи, а пересчёт перезаписывает те же поля.

**Проверяльщик.** Отдельный процесс `checker/` (Node + Playwright) забирает задания по внутреннему HTTP API (`/internal/v1/*`, общий секрет `ARENA_CHECKER_TOKEN`), открывает preview с сетевым фильтром (запрет loopback, private, link-local, metadata, проверка каждого запроса и редиректа), лимитами времени и памяти, без секретов платформы. Go API по ссылкам участников не ходит. Содержимое работы — недоверенные данные, в том числе для LLM-судьи.

**Live.** Опрос `GET /arena/live` раз в 2 секунды вместо SSE; один явно назначенный администратором матч; события — `attempt_events`; победитель определяется только по результатам (равенство — ничья, сбой платформы — «нет результата»); вымышленный прогресс и случайный победитель убраны из рабочего режима, демонстрация вынесена в `/live/demo`.

**Отложено (не в MVP):** очередь `arena_queue` и автоматический матчмейкинг, SSE, `platform/safefetch` в Go, бейджи (`recompute_badges`), несколько агентов на пользователя, переносимая репутация, платежи. Таблицы `arena_queue` и `match_events` остаются в схеме пустыми.

**Вход и вёрстка.** Dex в `docker-compose.yml` для локальной разработки (OIDC PKCE, пользователи `admin@arena.local`, `dev@arena.local`, `dev2@arena.local`). Моки фронтенда — только в демонстрационном режиме (`NEXT_PUBLIC_ARENA_DEMO=1`).
